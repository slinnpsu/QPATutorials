...
```{r Freq_Prop_table_for_wage_growth, exercise = TRUE, exercise.cap="Interactive exercise: Build a frequency and proportion table for wage_growth (freq and prop). Use the Run Code button to execute and see results."}
...
```{r compute_median_wage_growth, exercise = TRUE,exercise.cap="Interactive exercise: Compute the median of wage_growth. Use na.rm = TRUE."}

```

```{r compute_median_wage_growth-hint}
...
```{r compute_mean_wage_growth, exercise = TRUE,exercise.cap="Interactive exercise: Compute the mean of wage_growth Use na.rm = TRUE."}
...
```{r compute_mean_median_dem2p_percent, exercise = TRUE, exercise.lines=4,exercise.cap="Interactive exercise: Compute the median and mean of dem2p_percent. Use na.rm = TRUE."}

```

```{r compute_mean_median_dem2p_percent-hint}
...
```{r compute_range_min_max_wage_growth, exercise = TRUE,exercise.cap="Interactive exercise: Compute range(), min(), and max() for wage_growth. Use na.rm = TRUE."}
...
The 25th and 75th percentiles comprise what is known as the [inter-quartile range]{.important-text} or [IQR]. The IQR, splits the variable's values into four equal segments, or quartiles.  The advantage of the IQR as a measure of spread over the range is that any extreme values do not effect the IQR.
...
```{r calculate_percentile_employ_pop_ratio_25_64, exercise=TRUE,exercise.cap="Interactive exercise: List the default quantiles for employ_pop_ratio_25_64. Use the Run Code button to execute and see results."}

```

```{r calculate_percentile_employ_pop_ratio_25_64-hint-1}
Make sure whether you want the default settings or additional settings. 
```
...
```{r generate_histogram_wage_growth_custom_bin, exercise=TRUE,exercise.cap="Interactive exercise: Generate a histogram of wage_growth with 50 bins and white borders."}
ggplot(data = counties,  mapping = aes(x = wage_growth)) +
...
To set a custom bin width, we use bins = (numbers you want). 
```
...
```{r plot_vertical_and_text, exercise=TRUE,exercise.cap="Interactive exercise: Add a dotted red median line and text on the histogram."}
ggplot(data = counties,  mapping = aes(x = wage_growth)) +
         geom_histogram(color = "white", bins = 60) +
...
**Try it: plot tge distribution of the employment to population ratio using boxplot**
...
```{r histogram_dem2p_percent, exercise=TRUE,exercise.cap="Interactive exercise: Plot a histogram of dem2p_percent with blue outlines; optionally add red mean/median lines and labels."}
...
```{r histogram_dem2p_percent-hint-5}
...
```{r histogram_dem2p_percent-hint-6}
...
![](images/UnivariateInterval.png){width="70%" fig.alt="Infographic titled ‘Interval Level Variable’ with a left sidebar listing Central Tendency, Dispersion, Distribution Shape, and Visualization. The main panel shows four stacked blue boxes: Central Tendency—Mode, Median*, Mean**; Dispersion—Range, Percentiles (IQR), Standard Deviation, Variance; Distribution Shape—Unimodal, Bimodal, Multimodal, Uniform, Symmetric, Skewed, Normal; Visualization—Histogram, Box Plot."}

## Pulling what we've learned together, we have...


![](images/UnivariateDescription.jpg){width="70%" fig.alt="Infographic titled ‘Univariate Description’ with a left sidebar listing Central Tendency, Dispersion, Distribution Shape, and Visualization. Three main panels show stacked blue boxes: Interval Level Variable—Central Tendency (Mode, Median*, Mean**), Dispersion (Range, Percentiles/IQR, Standard Deviation, Variance), Distribution Shape (Unimodal, Bimodal, Multimodal, Uniform, Symmetric, Skewed, Normal), Visualization (Histogram, Box Plot); Nominal Level Variable—Central Tendency (Mode), Dispersion (Frequency or Proportion Distribution), Distribution Shape (Unimodal, Bimodal, Multimodal, Uniform), Visualization (Bar Plot of Frequencies or Proportions); Ordinal Level Variable—Central Tendency (Mode, Median), Dispersion (Frequency or Proportion Distribution), Distribution Shape (Unimodal, Bimodal, Multimodal, Uniform), Visualization (Bar Plot of Frequencies or Proportions)."}

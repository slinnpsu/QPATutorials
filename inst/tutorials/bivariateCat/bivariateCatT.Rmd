...
```{r}
library(sjPlot)
tab_xtab(counties$TrumpMajority, counties$rural, show.col.prc =TRUE) 
```
...
question("Which of the following statements are true?",
  answer("Of the countries with a regime type of Dictatorship, the level of gender equality with the highest proportion is "low"" (0.625)", correct=TRUE),
...
To plot proportions, we set an aesthetic inside the `geom_bar()` layer. Specifically, we set `y = after_stat(prop)` to represent the y axis as a proportion. We add `group = factor(rural)` so that `ggplot()` will calculate the proportions based on the number of cases in each value of the `fill` variable. 

...
* How to combine multiple [dplyr]{.package-name} operations with the pipe, `%>%`
...
* How to generate a bar plot of counts and proportions using `ggplot()` and `geom_bar()` from the [ggplot2]{.package-name} package
...
3. [**The shape of the distribution**]{.important-text}. The distribution of a variable is a summary of the frequency of individual values or ranges of values for a variable. When plotted, we can desribe the shape of the distribution.
...
To group by the values of **TrumpMajority** we first tell [dplyr]{.package-name} the name of the data frame that contains the variable "and then" using the pipe operator we pass the variable to the `group_by()` function as shown below. Note that we need to load the [dplyr]{.package-name} package with the `library()` function.
...
```{r analysis-practice-proportions, exercise = TRUE, exercise.cap="Interactive exercise: Create a proportion table. Fill in the XXXX placeholders and click Submit Answer to check correctness."}
counties %>%
  group_by(XXXX) %>%
  XXXX(freq = n()) %>%
  mutate(prop = XXXX)
...
```{r analysis-practice-freq-by-region, exercise=TRUE, exercise.lines=4, exercise.cap="Interactive exercise: Complete the code to summarize the data by region. Enter your code and click Submit Answer to check correctness."}
df %>%
  group_by(region) %>%
```

```{r analysis-practice-freq-by-region-hint}
...
The last piece of summary information concerns the shape of the distribution. 
...
2. [Bimodal]{.important-text}: It has two peaks, two categories are much more prevalent in the data
3. [Uniform]{.important-text}: All are distributed uniformly, there is an even spread of observations in each category
...
```{r visual-practice-bar-race, exercise = TRUE, exercise.cap="Interactive exercise: Create a bar chart for racial majority. Fill in the XXXX placeholders and click Submit Answer to check correctness."}
...
While the most frequently occurring category of **region** is "South," it is misleading to emphasize this fact. The number of observations (states) in each region is similar. Thus we might describe the shape of the distribution as fairly uniform.
...
```{r visual-practice-bar-proportion-regime, exercise = TRUE, exercise.eval = FALSE, exercise.cap="Interactive exercise: Create a proportion bar chart for regime type. Fill in the XXXX placeholders and click Submit Answer to check correctness."}
...
2. The appropriate tools for describing these features of our sample data depend on the level of measurement of the variable you wish to describe. The information here describes what is appropriate for nominal variables.
   ...
   a. The [**mode**] is the most commonly occurring category across cases and is the only statistic appropriate for describing the central tendency of a nominal variable. It may be more or less useful depending on the dispersion in the data, so often, we present the entire frequency distribution or visualize the distribution in a bar plot.
    b. The frequency distribution, or the distribution of proportions, reveals the [**dispersion**]{.important-text} in the data, i.e., it tells us how many or what proportion of cases take each value of the variable.  It also makes clear which category is the modal value and is a numeric representation of the [**dispersion**]{.important-text}.

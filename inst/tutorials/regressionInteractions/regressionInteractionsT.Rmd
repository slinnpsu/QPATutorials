---
title: "Multiple Regression: Conditional (or Interactive) Relationships with Interval-Level Variables"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
description: >
  Learn how to estimate and interpret conditional relationships involving 2 interval level variables.
---


## Learning Objectives

In this tutorial, you will learn how estimate and interpret interactive relationships involving interval-level variables using R. Specifically, we will cover:
...
Interpretation of interactive effects depends on the level of measurement of $X_1$ and $X_2$. Below we illustrate how to estimate and interpret interactive relationships when $X_1$ and $X_2$ are both measured at the interval level. Additional tutorials consider the cases where $X_1$ is measured at the interval level and $X_2$ is a binary nominal variables and where $X_1$ and $X_2$ are both binary nominal variables.
...
2. <span style="color:magenta">When are the conditional effects of each variable expected to be weakest and strongest?</span>
    ...
    b. We expect the conditional effect of percent white on the percent of the vote won by Hillary Clinton to be greater in counties that experience higher mortality risk if mortality risk makes white more anti-incumbent.
...
```{r lmcountiesX-setup, include = FALSE}
countiesX <- lm(dem2p_percent ~  mortality_risk_25_45 + wage_growth + mortality_risk_25_45*wage_growth + college_grad_percent + hispanic_percent + white_percent + black_percent, data=counties)
```


```{r plotcountiesX1, exercise = TRUE, exercise.setup="lmcountiesX-setup"}
library(sjPlot)
...
The shaded areas in the plot present confidence intervals around the predictions. Where these overlap, there is no discernible difference in the predicted values. Thus we can see that for wage growth between about -15 and -35,  mortality risk does not differ significantly in its effect on vote for Hillary Clinton. The effect of mortality differs significantly, however, as wage growth increases in a county.
...
```{r plotcountiesX1b, exercise = TRUE, exercise.setup="lmcountiesX-setup"}
...
```{r lmcountiesX2-setup, include=FALSE}
countiesX2 <- lm(dem2p_percent ~  mortality_risk_25_45 + wage_growth +mortality_risk_25_45*white_percent + college_grad_percent + hispanic_percent +  black_percent, data=counties)
```

```{r plotcountiesX2a, exercise = TRUE, exercise.setup="lmcountiesX2-setup"}
...
question("Examine the plot above and select the statements that are true",
  ...
  answer("The effect of mortality risk is significantly different for these three levels of **white_percent**", correct=TRUE, message="The effect of mortality risk on Hillary Clinton's vote depends on the percent white because the confidence intervals do not overlap. "),
  allow_retry = TRUE,
  try_again = "Hint: There is more than one correct answer"
  )
```

Write code to produce a plot illustrating the effect of the percent white conditioned on the value of mortality risk in a county and run the code.  Use values of **mortality_risk_25_45** of 1 (low risk), 3 (approximately mean risk), and 5 (high risk) to generate the plot.


```{r plotcountiesX2b, exercise = TRUE, exercise.setup="lmcountiesX2-setup"}
...
Interpreting the effects of interactions is best accomplished by using predicted values. The `plot_model()` function in the <span style="color:DarkGreen">sjPlot</span> package. The function will calculate the predicted values of the outcome variable (by setting the type argument to "pred") as a function of one variable (the first in the terms argument) conditional on the second variable (the second in the terms argument) in the interaction.  **Remember:** *Predictions are calculating after setting the values of all other variables at their mean values if interval (or ordinal treated as interval) and at their base value if they are categorical (nominal or ordinal).*  Ninety-five percent confidence intervals are included around the predictions, allowing you to see when predictions are significantly different from each other.

The results from these plots can be used to assess theoretical expectations about the nature of the conditional relationship. There are additional arguments that can be passed to the `plot_model()` function to generate predictions holding other variables are specific values. We'll cover these arguments in the context of logistic regression models. If your research question requires you to estimate conditional relationships you should check out http://www.strengejacke.de/sjPlot/articles/plot_interactions.html for additional examples and options for plotting predicted values with `plot_model()`.

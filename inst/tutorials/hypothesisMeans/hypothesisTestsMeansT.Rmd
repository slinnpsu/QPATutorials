...
In this tutorial, you will learn how conduct hypothesis tests about means. Specifically, we will cover:
...
A <span style="color:blue">one sample means test</span> is used to test whether the mean of a single variable $Y$ in a single sample is equal to a hypothesized value. 
...
To compute a one-sample $t$ test, we use the `t.test()` function. The function requires we pass it the name of the variable for which to calculate the sample mean and the argument `mu`, which gives the value under the null hypothesis. Running the following code will produce the value of the t-test, the corresponding p-value, a confidence interval for the mean, and the sample mean.
...
R reports the two-tailed p-value associated with the t-statistic in scientific notation as "p-value < 2.23-16."  This is 2.23 to the negative 16th or 0.000000000000000223. When the p-value is this small, we report p<0.001.

**Step 5. Draw a conclusion.**

```{r letter-c, echo=FALSE}
question("Which of the following statements are true?",
  answer("Because the p-value is less than 0.05, we can reject the null hypothesis with 95% confidence", correct=TRUE),
  answer("Because the p-value is less than 0.05, we cannot reject the null hypothesis with 95% confidence",  message="Remember if p is low the null must go!"),
  answer("Because the absolute value of the t-statistic is greater than 1.96, we can reject the null hypothesis with 95% confidence", correct=TRUE),
  answer("Because the confidence interval does not include the null value, we can reject the null hypothesis with 95% confidence", correct=TRUE),
...
A <span style="color:blue">difference of means test</span> involves a comparison of the means in two groups. We focus here on the difference of means in two independent samples. This means that there is no overlap in the membership of cases in each sample.
...
To test whether the mean value of a single variable, $Y$, is the same in two different populations (in two groups) against the alternative that it is different, we need to use what R calls a formula. We list the name of the (interval level) variable for which we want to calculate the mean, then we type a tilde (~) and then the name of the variable that defines the two groups: `t.test(X ~ GroupVariable)`. If the variable resides in two different data objects use: `t.test(DF1$variable, DF2$variable)`.

If we wish to test the null hypothesis against a one-tailed alternative -- that the mean is higher or lower in one of the two groups -- we add the `alternative` argument, setting it to `"greater"` if the mean in group one is hypothesized to be greater  than in group two and `"less"` if the mean in group one is hypothesized to be smaller than in group two. The default is a two-tailed alternative hypothesis, so you don't need to specify this argument if the alternative hypothesis is two-tailed.

The variables **TrumpMajority** (coded 0 if Hillary Clinton won a majority of the county's vote and 1 if Donald Trump won a majority) and **prop_black** (the proportion of the county residents who are Black) in the **counties** data frame contain the information needed to test this hypothesis.

Note that we specify `alternative="greater"` because the first group mean computed is for the smallest value of **TrumpMajority**, 0, which denotes counties Clinton won. Anytime you use a one-tailed test, take care to ensure you choose the appropriate alternative given the coding of the variables!
...
question("Which of the following statements are true?",
  ...
  answer("The mean proportion of the vote won by counties that were majority black was 0.23", message="0.23 is the proportion black in counties won by Hillary Clinton. "),
...
```{r paired2-solution}
t.test(world$gini10, world$gini08, alternative="greater")
```
...
question("Which of the following statements are true?",
  ...
  answer("There is a 66.97 percent chance the mean of the difference  between the two Gini coefficients is 40.64754-40.12941=0.51813", message="It is not true that the p-value tells us the probability that the estimated mean difference is true. Instead, it tells us that there is a 66.97 percent chance we'd observe the mean of the difference between the two Gini coefficients as great as we observe if the null hypothesis is true."),
  allow_retry = TRUE,
  try_again = " "
  )
```
...
+ Use the `summarise()` function to calculate the mean in each group using `mean=mean()`. This is the numerator in the formula for the t-statistic. We also need to calculate the number of cases in each group using `n=n()`; and the standard deviation for each group using `sd=sd()`. This information is needed to calculate the denominator in the formula for the t-statistic.
+ Use the `mutate()` function to calculate the standard error using  `se=2*(sd/sqrt(n))`. This is the denominator in the formula for the t-test.
...
In the summarize function, take the mean of dem2p_percent.

Don't forget to remove any missing values with the 
na.rm argument.

Set sd=sd(demp2p_percent).

Then use the mutate function to generate the standard error by setting 
se=sd/sqrt(n)).
```

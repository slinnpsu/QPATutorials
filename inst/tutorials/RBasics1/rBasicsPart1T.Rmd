...
<style>
/* Only show focus indicators when actually focused */
...
/* Override any conflicting learnr cursor rules */
[data-label] .btn, 
...
It is easy to use R like a calculator. Here's an example of adding 2 numbers. You can add/subtract, multiply/divide, or raise a number to a power using `+`, `-`, `*`, `/`, `^`. 

**Practice: Basic arithmetic in R**
Try running the code below, then change the numbers and operators to practice different calculations. Note that R ignores spaces, so 14 + 2000 will produce the same output as 14+2000.

```{r calculator-practice, exercise=TRUE, exercise.cap="Practice basic arithmetic operations in R"}
14 + 2000
```

The `[1]` you see with the line of output is just R telling us that we have just one number produced by the code. Later we will see cases where a command we run produces an output that contains multiple numbers (or words). Each line of output will begin with some number in square brackets [ ], with the number denoting the index value of the output that starts that line.
...
**Practice: Create your first object**
Let's assign the sum of 5+3 to an object called **my.first.object**. Run the following code to see how assignment works.

```{r objects-create-first, exercise=TRUE, exercise.cap="Create and assign a value to your first R object"}
my.first.object <- 5 + 3
```
...
**Practice: Display an object's value**
Run this code to see how to display what's stored in an object:

```{r objects-print-value, exercise=TRUE, exercise.cap="Display the value stored in an R object"}
my.first.object <- 5 + 3
my.first.object
```
...
**Practice: Reassign an object**
Run the code to see what happens when you assign a new value to an existing object:

```{r objects-reassign, exercise=TRUE, exercise.cap="Practice reassigning a new value to an existing object"}
...
**Practice: Test case sensitivity**
Run this code to see what error occurs with incorrect capitalization:

```{r objects-case-error, exercise=TRUE, exercise.cap="Observe error message from incorrect object name capitalization"}
MY.FIRST.OBJECT
```
...
**Practice: Create a character object**
See how to store text as an object using quotation marks:

```{r objects-create-string, exercise=TRUE, exercise.cap="Create an object containing character text using quotation marks"}
this.class <- "PLSC 309"
this.class
```
...
**Practice: Numbers as characters**
Observe what happens when you put numbers in quotation marks:

```{r objects-number-as-string, exercise=TRUE, exercise.cap="Store numbers as character text using quotation marks"}
another.object <- "100"
another.object
```
...
**Practice: Arithmetic error with character strings**
Run this code to see the error when trying arithmetic with character strings:

```{r objects-string-arithmetic-error, exercise=TRUE, exercise.cap="Observe error when attempting arithmetic operations on character strings"}
another.object <- "100"
another.object / 10
```
...
The function `sqrt()` takes one argument, named `x`, which is the number or object for which we want the square root. 

**Practice: Calculate a square root**
Try taking the square root of 4:

```{r functions-square-root, exercise=TRUE, exercise.cap="Calculate the square root of 4 using the sqrt function"}

```

```{r functions-square-root-hint}
Type the function name sqrt, then put the number 4 inside parentheses.
```
...
To learn the arguments accepted by a function, call the `args()` function and pass it the name of the function (without parentheses). Below I've passed `args()` the function for standard deviation: `sd`. 

**Practice: Explore function arguments**
Run the code below to see how many arguments this function has and what they are:

```{r functions-explore-args, exercise=TRUE, exercise.cap="Examine the arguments accepted by the sd function"}
args(sd)
```
...
**Practice: Different ways to call functions**
See how these three function calls produce the same result:

```{r functions-argument-order, exercise=TRUE, exercise.cap="Practice calling functions with arguments in different orders"}
sd(num.list, FALSE)
...
**Practice: Access function help**
Try getting help documentation for the sd function:

```{r functions-get-help, exercise=TRUE, exercise.cap="Access help documentation for the sd function"}
?sd
```
...
Note: If you receive an error message like the following, then the function is likely located in a package you have not loaded: "No documentation for 'funcname' in specified packages and libraries: you could try '??funcname'". Typing ??funcname will cause R Studio to look more broadly for the function and indicate which packages contain that function. You can try this option or you can install the relevant package (if you have not), load it with library(package_name), and then type ?funcname in the console window.
...
```{r classes-determine-types, exercise=TRUE, exercise.setup = "prepare-classes", exercise.cap="Use the class function to determine the type of different R objects"}

```

```{r classes-determine-types-hint}
You need three separate lines of code. Each line should use the class() function with a different object name inside the parentheses. The three objects are: my.first.object, another.object, and sqrt.
```
...
```{r vectors-create-first, exercise=TRUE, exercise.cap="Create a vector with 5 numbers using the c function"}
...
**Table: Presidential Election Results 1980-2020**
...
```{r vectors-election-data, exercise=TRUE, exercise.cap="Create a vector containing Democratic vote shares from the election data table"}

```

```{r vectors-election-data-hint}
You need to:
1. Create an object called dem.pres.vote.share using the assignment operator <-
2. Use the c() function to combine all 11 vote share values from the table
3. Separate each number with commas
4. Print the object by typing its name on a new line
```
...
```{r vectors-combine-names, exercise=TRUE, exercise.cap="Create and combine two string vectors containing candidate names"}

```

```{r vectors-combine-names-hint}
Break this into three steps:
1. Create first.5.candidates using c() with the first 5 candidate last names in quotes
2. Create last.6.candidates using c() with the remaining 6 candidate last names in quotes  
3. Combine them using c() and assign to dem.candidates
4. Print dem.candidates to check your work
```
...
Modify this code to extract Hillary Clinton's vote share (the 10th element):

```{r vectors-access-single, exercise=TRUE, exercise.cap="Extract a specific element from a vector using index notation"}
dem.pres.vote.share[1]
```

```{r vectors-access-single-hint}
Hillary Clinton's data is the 10th element in the vector. Change the number inside the square brackets from 1 to the correct index number.
```
...
```{r vectors-access-multiple, exercise=TRUE, exercise.setup="vectors-election-data", exercise.cap="Extract multiple elements from a vector using the c function within brackets"}

```

```{r vectors-access-multiple-hint}
The last 5 elections are elements 7, 8, 9, 10, and 11. Use square brackets with c() inside to select these index numbers.
```
...
**Practice: Order of vector elements**
See how the order of index numbers affects the output:

```{r vectors-order-matters, exercise=TRUE, exercise.setup="vectors-election-data", exercise.cap="Observe how the order of index values affects vector output"}
...
**Practice: Remove vector elements**
See how to exclude specific elements from a vector using negative indexing:

```{r vectors-remove-elements, exercise=TRUE, exercise.cap="Remove specific elements from a vector using negative indexing"}
...
```{r vectors-convert-percent, exercise=TRUE, exercise.setup="vectors-election-data", exercise.cap="Convert vote shares to percentages by multiplying by 100"}

```

```{r vectors-convert-percent-hint}
Multiply dem.pres.vote.share by 100 and assign the result to dem.pres.vote.percent. Don't forget to print the result.
```
...
Create a vector showing how each election's Democratic vote share compares to 1980:

```{r vectors-compare-baseline, exercise=TRUE, exercise.setup="vectors-election-data", exercise.cap="Calculate the difference between each election and the 1980 baseline"}

```

```{r vectors-compare-baseline-hint}
The 1980 vote share is the first element of the vector (index 1). Subtract dem.pres.vote.share[1] from the entire dem.pres.vote.share vector.
```
...
Can you figure out how to replace the three "Clinton" entries with their full names? (Remember to check your work by printing your object)

```{r vectors-replace-elements, exercise=TRUE, exercise.setup="vectors-combine-names", exercise.cap="Replace specific vector elements using index assignment"}

```

```{r vectors-replace-elements-hint-1}
First, identify which positions in dem.candidates contain "Clinton". Look at the candidate list - Clinton appears in positions 4, 5, and 10.
```

```{r vectors-replace-elements-hint-2}
Use square brackets with c() to specify the positions [c(4,5,10)], then assign the replacement values using c() with the full names in quotes.
```
...
**Practice: Apply summary functions**
Below I've applied each function to some (numeric!) vector we created with our vote data:

```{r functions-summary-stats, exercise=TRUE, exercise.cap="Apply various summary functions to vector data"}
...
**Practice: Create number sequences**
Here's an example that creates a sequence of numbers from 1 to 10 by increments of 2 and checks our work:

```{r functions-create-sequence, exercise=TRUE, exercise.cap="Create a numeric sequence using the seq function with specified intervals"}
my.sequence <- seq(from = 1, to = 10, by = 2)
my.sequence
```

Let's create an object for the "election year" variable in our table using this function.

**Practice: Create election year sequence**
Name your object **elect.year** and print the result:

```{r functions-election-years, exercise=TRUE, exercise.cap="Create a sequence of election years from 1980 to 2020"}

```

```{r functions-election-years-hint}
Election years run from 1980 to 2020, with elections every 4 years. Use seq() with from=1980, to=2020, and by=4.
```
...
**Practice: Create decreasing sequences**
See how to create a sequence that counts backwards:

```{r functions-decreasing-sequence, exercise=TRUE, exercise.cap="Create a decreasing numeric sequence using negative by values"}
seq(from = 2020, to = 1980, by = -4)
```
...
**Practice: Assign and use names**
See how to add descriptive names to vector elements:

```{r functions-assign-names, exercise=TRUE, exercise.setup="functions-prepare-names", exercise.cap="Add descriptive names to vector elements using the names function"}
...
**Practice: See package error**
Observe what happens when you try to use a function from an unloaded package:

```{r packages-error-demo, exercise=TRUE, exercise.cap="Observe error message when trying to use function from unloaded package"}
ggplot()
```
...
**Practice: Load package and use function**
See how loading a package makes its functions available:

```{r packages-load-demo, exercise=TRUE, exercise.cap="Load a package using library and then access its functions"}
library(ggplot2)
ggplot()
```
...
**Practice: Get package help**
Learn how to access comprehensive help for an entire package:

```{r packages-get-help, exercise=TRUE, exercise.cap="Access comprehensive help documentation for an R package"}
...
**Practice: Use code comments**
See how comments can document your code without affecting execution:

```{r comments-demo, exercise=TRUE, exercise.cap="Add explanatory comments to R code using hashtag symbols"}
#Sample comment
...
```{r practice-republican-shares, exercise=TRUE, exercise.cap="Calculate Republican vote shares as one minus Democratic shares"}

```

```{r practice-republican-shares-hint}
Since only two parties are considered, Republican share equals 1 minus Democratic share. Use 1 - dem.pres.vote.share and assign it to rep.pres.vote.share.
```
...
```{r practice-victory-margins, exercise=TRUE, exercise.setup="practice-setup-for-margins", exercise.cap="Calculate victory margins using absolute differences between party vote shares"}

```

```{r practice-victory-margins-hint}
The victory margin is the absolute difference between the two parties' vote shares. Use abs() around the difference between rep.pres.vote.share and dem.pres.vote.share.
```
...
```{r practice-republican-names, exercise=TRUE, exercise.cap="Create a vector containing Republican presidential candidate last names"}

```

```{r practice-republican-names-hint}
Use the c() function with the Republican candidate last names in quotes. Check the table earlier in the tutorial if you need to remember the names.
```
...
```{r practice-mean-republican, exercise=TRUE, exercise.setup="practice-setup-for-margins", exercise.cap="Calculate the mean Republican vote share across all elections"}

```

```{r practice-mean-republican-hint}
Apply the mean() function to the rep.pres.vote.share vector.
```
...
```{r practice-range-republican, exercise=TRUE, exercise.setup="practice-setup-for-margins", exercise.cap="Find the minimum and maximum Republican vote shares using the range function"}

```

```{r practice-range-republican-hint}
Apply the range() function to the rep.pres.vote.share vector.
```
...
<strong>Congratulations!</strong> Way to go! You've finished the first tutorial.
</div>

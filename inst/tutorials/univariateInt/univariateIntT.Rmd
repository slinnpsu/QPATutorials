...
This code will produce a warning message. Not to worry.
...
Notice that there is a warning message associated with the plot. "Pick better value with `binwidth`." `ggplot()` will choose a default number of bins based on a formula. Here, it picked 30 and gave us a warning that we might wish to consider alternatives. To do so, we add `bins` argument to `geom_histogram()`. Try experimenting with the number of bins in our plot by augmenting the code and trying different values.
...
Try to produce a histogram without looking at the hints!  If you are feeling like a challenge, add the median and mean value to the plot and label it.

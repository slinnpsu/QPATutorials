...
```{r wagetable-hint-1}
Remember we start with then data frame name 'and then' use group_by, summarise, and mutate in conjunction with the pipe
operator.
```
...
2. Observations outside that range are indicated by circles. Here we see that several counties have more than $100\%$ of adults age 25-54 who are employed. (These counties must be "importing" workers from other counties.)

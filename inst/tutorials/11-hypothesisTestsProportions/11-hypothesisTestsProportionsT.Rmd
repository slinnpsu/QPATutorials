...
* How to use the `prop.test()` function to conduct (1) [one-sample proportions tests]{.important-text}, and (2) [difference of proportions tests involving two independent samples]{.important-text}. 
* How to use the `CrossTable()` function in the [descr]{.package-name} package to test whether two nominal and/or ordinal variables are independent.
...
library(dplyr)
data("aNES", package = "qpaTutorials")
aNES <- aNES %>% 
...
In this tutorial we will cover methods for conducting [one-sample proportion tests]{.important-text}, [two-sample (difference of) proportions tests]{.important-text}, and the [$\chi^2$ test for independence]{.important-text}.



## One-sample proportion tests 

The simplest form of hypothesis test involving proportions is a one-sample proportion test. A [**one-sample proportion test** tests whether the proportion of cases taking some value in a sample is equal to a specific value against the alternative that it is not equal to that value.]{.important-text} We use the `prop.test()` function in R to test this hypothesis.
...
We will use the `group_by()` and `summarisz()` functions in the [dplyr]{.package-name} package to generate the frequency table. Recall that to use these [dplyr]{.package-name} functions we must load the package with the `library()` function first. Complete the code below to produce the frequency table.  Name your new variable **counts**.
...
A [**difference of proportions test** tests whether a proportion is the same in two (independent) groups (or samples).]{.important-text} Like the one-sample proportion test, we can use the `prop.test()` function to conduct this test.
...
```{r ideology-solution}
prop.test(x = c(85, 58), n = c(85 + 288 + 202 + 469 + 221 + 320 + 76, 58 + 211 + 173 + 415 + 282 + 374 + 90))
```
...
question("Look at the prop.test output. Which of the following is true?",
  ...
  answer("We can be 95% confident the true difference in proportions in the population is between about 0.0004 and 0.0296", correct=TRUE),
  answer("Males are more likely to say they are very liberal than females", message = "Because the proportion of females who are very liberal is about 0.051 and the proportion of males is about 0.036, females are more likely to say they are very liberal"),
  allow_retry = TRUE,
  try_again = "Hint: Make sure to select all true statements."
  )
```
...
We will use the **counties** data frame, which contains data on counties in the United States in 2016, to test whether the proportion of majority white counties Donald Trump won was different from the proportion of non-majority white counties he won. The two variables of interest are **White**, which takes the values "Majority White" and "Non-Majority White", and **TrumpMajority**, which takes the value 0 if Hillary Clinton won the county and 1 if Donald Trump won the county.


**Step 1. Formulate the null hypothesis**. The proportion of majority white counties Trump won is equal to the proportion of non-majority white counties Trump won: $$H_0: \pi_w=\pi_{NW}$$ 


**Step 2. Formulate the alternative hypothesis**. The proportion of majority white counties Trump won is **not** equal to the proportion of non-majority white counties Trump won: $$H_A:\pi_w \ne \pi_{NW}$$
...
The final hypothesis test we will cover involving proportions is the [**$\chi^2$ test for independence**, which tests whether two categorical variables are independent (unrelated) or whether there is some relationship between them.]{.important-text}

The logic of the test is based on whether the number of observed counts in the cells of a cross tab is different than what would be expected if there was not a relationship between the two variables. This test is available as an argument to the `CrossTable()` function in the [descr]{.package-name} package.  Remember to load this package with the `library()` function and to list the dependent variable first when specifying the `CrossTable()` function. 
...
For our final exercise, we will investigate whether there is a relationship between a person's decision to watch Modern Family and their vote choice in the sample of respondents in the American National Election Study.  
...
Hypothesis tests involving categorical variables, whether nominal or ordinal, involve proportions. [**One-sample proportion tests**]{.important-text} allow us to compare a sample proportion to some value. [**Two-sample or difference of proportion tests**]{.important-text} allow us to compare the same proportion but in two independent groups to determine whether they differ.  Finally, the [**$\chi^2$ test of independence**]{.important-text} allows us to test whether two categorical variables are independent of one another or whether the values taken by one variable predict those taken by another.
...
To implement the $\chi^2$ test for independence in R we use the `CrossTable()` function in the [descr]{.package-name} package. We simply set the argument `chisq = TRUE`. If we have a small sample, Fisher's exact test should be used instead. To implement this test we use the `CrossTable()` function in the [descr]{.package-name} package but set the argument `fisher = TRUE`. 

```{r tutorial-success-banner-11, results="asis", echo=FALSE}
cat('
<p role="status"
   aria-label="Impressive! You have completed this tutorial and can now conduct hypothesis tests involving proportions."
   style="font-size: 28px; font-weight: 600; color: #2e7d32; text-align: center; margin-top: 50px;">
  <span aria-hidden="true">🎓 Impressive work! 🎓</span>
</p>
')
```

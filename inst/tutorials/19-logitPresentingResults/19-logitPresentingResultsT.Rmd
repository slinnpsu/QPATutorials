...
* How to use the `plot_model()` function in the  [sjPlot]{.package-name} package to produce predicted values from a logistic regression analysis. Specifically, we will cover how to plot the effects of one or more variables and how to set the values of independent variables not being plotted.
...
data("states2020", package = "qpaTutorials")
...
Note that we need to load the [stargazer]{.package-name} package using the `library()` function before we can use the `stargazer()` function.
...
::: {.caution}
You will need to be careful to make sure you list the variable labels in the correct order, starting with the first explanatory variable listed in the `glm()` function. Always look at the summary first and compare your stargazer table to that output after you produce it!
:::
...
* We can use the `stargazer()` function in the [stargazer]{.package-name} package to present tables of results from a logistic regression that include titles,  assign meaningful names to all variables, and provide the information necessary for your reader to assess the statistical significance of the variables in your model.  See ?stargazer for additional details.
* We can  use the `plot_model()` function in the [sjPlot]{.package-name} package to present coefficient plots of the results from a logistic regression. We specify the `type` of plot as `"est"`. A number of additional arguments can be specified to control the look and feel of the plot. See ?plot_model for additional details.
* We can also use the `plot_model()` function in the [sjPlot]{.package-name} package to present predicted value plots. We specify the `type` of plot as `"pred"`. We can plot predictions from a model as a function of one, two, or three variables.
...
cat('
<p role="status"
   aria-label="Outstanding! You have completed this tutorial and can now present logistic regression results professionally using tables and predicted value plots."
   style="font-size: 28px; font-weight: 600; color: #2e7d32; text-align: center; margin-top: 50px;">
  <span aria-hidden="true">🏆 Outstanding work! 🏆</span>
</p>
')
```

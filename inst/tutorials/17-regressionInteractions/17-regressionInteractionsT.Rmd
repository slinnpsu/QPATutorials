...
* How to interpret results from multiple regression analysis with interactions using the `plot_model()` function in the  [sjPlot]{.package-name} package to produce plots of predicted values.
...
library(rio)
data("trade", package = "qpaTutorials")
...
options(scipen = 0)
options(width = 80)
data("states", package = "qpaTutorials")
...
1. [Do you expect the conditional effect of a variable to be negative or positive for all values of the conditioning variable?]{.important-text} 
...
2. [For what values of the conditioning variable do you expect its effect to be biggest and smallest?]{.important-text} 
...
1. [Do we expect conditional effects of each variable to always be positive or negative? ]{.important-text}
...
2. [When are the conditional effects of each variable expected to be weakest and strongest?]{.important-text}
...
1. [Do we expect the conditional effects of each variable to always be positive or negative?]{.important-text}
    a. We expect the conditional effect of mortality risk to be negative for all values of wage growth because higher mortality risk is never a good thing.
...
cat('
<p role="status"
   aria-label="Impressive! You have completed this tutorial and can now estimate, interpret, and visualize conditional relationships using interaction terms."

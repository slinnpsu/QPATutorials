...
To plot proportions, we give the `geom_bar()` function its own aesthetic, which formats the y-axis. Specifically, we include ``aes(y = after_stat(count/sum(count)))` inside the `geom_bar()` function. This transforms the y-axis to a proportion: `geom_bar(aes(y = after_stat(count/sum(count))))`.
...
3. Have you specified y= after_stat(count/sum(count)) inside the geom_bar aes() function?
4. Have you specified fill="red" outside aes() in geom_bar?
5. Did you set the y label to "Proportion"?
6. Did you specify the scale_x_discrete function to edit the x axis?
```

```{r plotraceprop-solution}
ggplot(data = world, mapping = aes(x = factor(dem_level4))) +
  geom_bar(aes(y = after_stat(count/sum(count))), fill = "red") +
  labs(title = "Regime Type",
       subtitle = "The Economist",
       x = NULL,
       y = "Proportion") +

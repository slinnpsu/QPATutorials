...
* How to use the `stargazer()` function in the  [stargazer]{.package-name} package to produce professional looking tables.
* How to produce a plot of the coefficient estimates as an alternative to a table using the `plot_model()` function in the  [sjPlot]{.package-name} package.
* How to use the `plot_model()` function in the  [sjPlot]{.package-name} package to produce predicted values from a regression analysis.
...
The `summary()` function presents results from a regression analysis, but the output format is not informative to readers. There is no title and variable names are typically only meaningful to the analyst who created them. We want to create tables that look like those you see in articles and books. To do so we will use the `stargazer()` function from the [stargazer]{.package-name} package.
...
Note that we need to (install and) load the [stargazer]{.package-name} package using the `library()` function before we can use the `stargazer()` function.
...
::: {.caution}
You will need to be careful to make sure you list the variable labels in the correct order, beginning with the first variable listed in the `lm()' function and ending with the intercept. For categorical variables with more than two categories each of the included categories must get a separate label. Always look at the summary first and compare your stargazer table to that output after you produce it!
:::
...
A picture is worth a 1000 words, so let's present our results in a coefficient plot. We will be using the `plot_model()` function in the  [sjPlot]{.package-name} package. Load the package with `library(sjPlot)` before you use it. 
...
The remaining options replace the default labels in the plot. ::: {.caution}
Note: The label arguments are different in `plot_model()` than when using `ggplot()`.
:::
...
It is important to present findings from your regression analysis professionally. Tables that include titles,  assign meaningful names to all variables, and provide the information necessary for your reader to assess the statistical significance of the variables in your model are important. Measures of fit should always be reported as well. The `stargazer()` function in the [stargazer]{.package-name} package provides one option for producing tables. 

Sometimes we prefer a  plot to present coefficient estimates, but the text should then clearly identify measures of model fit.  The `plot_model()` function in the [sjPlots]{.package-name} package can be used to produce coefficient plots. We specify the `type` of plot as `"est"` A number of additional arguments can be specified to control the look and feel of the plot.
...
cat('
<p role="status"
   aria-label="Superb! You have completed this tutorial and can now present regression results professionally using tables and coefficient plots."

...
library(dplyr)
aNES <- counties <- qpaTutorials::cleaned_ANES
aNES <- aNES %>% 
...
**1. Formulate the null hypothesis**. $$H_0: \pi=0.489$$ The proportion of voters who voted for Trump in the ANES sample ($p$) is consistent with the population value of 0.489.

**2. State the alternative hypothesis**. $$H_A: \pi \ne 0.489$$ The proportion of people in the ANES sample that voted for Trump ($p$) is not equal to that in the population (0.489).
...
Write the code to generate the counts needed to calculate proportions. You do not need to eliminate missing values since there are few categories. Make sure to name your count variable **counts**.
...
**Step 1. Formulate the null hypothesis**. Under the null hypothesis there is no relationship between whether a person watches Modern Family and from whom they voted for president in the population. This implies that knowing whether or not someone watched Modern Family will not help us predict their vote choice.
...
To implement the $\chi^2$ test for independence in R we use the `CrossTable()` function in the <span style="color:DarkGreen">descr</span> package. We simply set the argument `chisq=TRUE`.

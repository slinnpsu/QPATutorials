...
We will use the `group_by()` and `summarisz()` functions in the <span style="color:DarkGreen">dplyr</span> package to generate the frequency table. Recall that to use these <span style="color:DarkGreen">dplyr</span> functions we must load the package with the `library()` function first. Complete the code below to produce the frequency table.  Name your new variable **counts**.
...
A <span style="color:blue">**difference of proportions test** tests whether a proportion is the same in two (independent) groups (or samples).</span> Like the one-sample proportion test, we can use the `prop.test()` function to conduct this test.

...
We will also practice the tools we used to generate frequency distributions and proportions for nominal variables using the functions in the package <span style="color:DarkGreen">dplyr</span>, as well as bar plots with the `ggplot()` function in the package <span style="color:DarkGreen">ggplot2</span>.
...
question("Which of the following variables are ordinal level measures?",
  ...
  answer("The level of education of a respondent in a survey where the values include less than high school degree, high school degree, college degree, and beyond college", correct = TRUE, message="Level of education is ordinal because each successive value indicates more education, but the differences between the categories are not the same. "),
...
We can calculate the central tendency of an ordinal variable using the mode, but in addition, 
...
See if you can create a frequency and proportion table for the variable **rural_urban** in the dataframe **counties** using the <span style="color:DarkGreen">dplyr</span> functions. Name your frequency variable `freq` and your proportion variable `prop`.  Don't forget to load the package with the `library()` function!
...
2. The number of counties in many categories is somewhat similar. The data is highly disbersed.
3. The distribution is somewhat uniform. This makes the mode less informative about the typical county’s rural-ness than if one category dominated the data, in which case the data would be unimodal. If half the counties were in very urban and half very rural, the distribution would be bimodal. These distinctions are somewhat subjective so we usually report the frequency (proportion) distribution or the bar plot and let the reader judge.

Note that we cannot easily determine the median from the plot.

### Your turn

Let’s look at another ordinal variable. We will examine the variable **FHStatus**, Freedom House's measure of whether a country is "Not Free" (coded 0), "Partly Free" (coded 1), or "Free" (coded 2). The variable is in the data set **fHouse**.  The variable is ordinal rather than categorical because as you go from "Not Free" to "Free" the country becomes more *free*. The variable constitutes a ranking of countries on this dimension.

First we will generate a frequency and proportion table using the <span style="color:DarkGreen">dplyr</span> functions.  You do not need to load the package again because we've done so in an earlier exercise.  See if you can produce the table without looking at the hints. 
...
question("Which of the following statements *best* describes the mode of the variable FHStatus?",
  ...
  answer("There distribution is multi-modal", message="One single category is more prevalent than all others,"),
  allow_retry = TRUE
)
```
...
put both the old and new names in quotations and to
separate them with commas and place them inside c()?
```
...
ggplot(data=fHouse,  mapping=aes(x=factor(FHStatus))) +
         ...
         labs(title = "Freedom House Ranking of Countries", 
              x=NULL,  
              y="Frequency") +
        scale_x_discrete(na.translate=FALSE, labels=c("0"="Not Free","1" = "Partly Free", "2"="Free"))
```

...
* How to conduct descriptive analysis of the relationship between nominal or ordinal variables and an interval variable by comparing means across categories.
* How to use the functions in the R package <span style="color:DarkGreen">dolyr</span> to produce means by categories of a nominal/ordinal variable.
...
We would expect the mean change in mortality risk (**mortality_risk_25_45_change**) for counties that cast a majority of votes for Donald Trump would to be greater than in those counties that cast a majority of votes for Hillary Clinton.  Edit the XXXX in the code below to produce the mean of the change in mortality risk for counties won by Clinton and Trump.
...
As a final example, consider the hypothesis that <span style="color:chocolate"> In a comparison of [countries] those that [are more democratic] will be more likely to  [have better economic outcomes] than will those that are [less democratic].</span> 

Calculate the means of the variable **gdp08**,
GDP in billions in 2008 (World Bank) by **dem_level4**,
Regime type (Economist 2014), in the data frame **world**. Try not to look at the hints!
...
The table of means tell us that the mean is highest, 1394.33, among full democracies ("Full Democ"), followed by 354.79 among partial democracies ("Part Democ"), as we would expect, but authoritarian governments have the next highest means at 320.43, and finally hybrid democracies has the lowest GDP.
...
ggplot(mapping=aes(x=factor(TrumpMajority), y=mean)) +
  ...
  labs(title="Mean Change in Wage Growth Among Counties \nCasting a Majority of Votes for Clinton and Trump",y="Change in Mortality Risk", x=NULL, caption="Source: Linn, Nagler, and Zilinsky") +
  scale_x_discrete(labels = c("0"="Clinton", "1"="Trump")) +
  scale_y_continuous(limits=c(-10,10)) 
```

Try to complete the code below to plot the mean **mortality_risk_25_45_change** for counties Trump won and Clinton won by completing the code.
...
argument to c("0"="Clinton", "1"="Trump") and
...
counties %>%
  group_by(TrumpMajority)%>%
  summarise(freq=n(),mean=mean(mortality_risk_25_45_chang, na.rm=TRUE)) %>%
ggplot(mapping=aes(x=factor(TrumpMajority), y=mean)) +
    geom_point(size=3,shape=19) +
...
Finally, try to produce the plot of mean GDP, **gdp08**, by level of democracy, **dem_level4** from the **world** data frame. Add titles to your plot. You do not need to adjust the y-axis limits.
...
world %>%
  ...
  labs(title="Mean GDP by Level of Democracy",
       x=NULL,
       y="Mean GDP") +
  scale_x_discrete(labels=c("Full Democ"="Full \nDemocracy", "Part Democ"="Partial \nDemocracy", "Hybrid"="Hybrid", "Authoritarian"="Autoritarian"))
```
...
## Visualizing the distributions of an interval level variable based on categories of a nominal or orinalnal variable
...
ggplot(data=world, mapping=aes(x=dem_level4, y=lifeex_total, fill=dem_level4)) +
  ...
  labs(title="Distribution of Life Expectancy at Birth", 
       x="Democracy Level", 
       y="Frequency")  +
  scale_x_discrete(labels=c("Full Democ"="Full Democracy", "Part Democ"="Partial Democracy", "Hybrid"="Hybrid", "Authoritarian"="Authoritarian")) +
guides(fill=FALSE)
```

...
```{r star, warning=FALSE, message=FALSE, exercise=TRUE, exercise.setup="model1-setup"}
library(stargazer)
stargazer(model1,
          type = "text")
```
...
```{r star2, warning=FALSE, message=FALSE, exercise=TRUE, exercise.setup="model1-setup"}
library(stargazer)
stargazer(model1,
          type = "text",
...
```{r model2-setup, include=FALSE}
model2 <- lm(dem2p_percent ~ mortality_risk_25_45 + wage_growth + college_grad_percent + hispanic_percent + white_percent + black_percent + factor(UrbanCat), data=counties)
summary(model2)
```


```{r star3, warning=FALSE, message=FALSE, exercise=TRUE, exercise.setup="model2-setup"}
stargazer(XXX,
          XXX = "XXX")
```
...
```{r star4, warning=FALSE, message=FALSE, exercise=TRUE, exercise.setup="model2-setup"}
...
```{r coeffplot1, warning=FALSE, message=FALSE,exercise=TRUE, exercise.setup="model1-setup"}
library(sjPlot)
plot_model(model1, 
           type = "est")
```
...
```{r coeffplot2, warning=FALSE, message=FALSE, exercise=TRUE, exercise.setup="model1-setup"}
library(sjPlot)
plot_model(model1, 
           type = "est",
           show.values = TRUE, 
...
```{r coeffplot2a, warning=FALSE, message=FALSE, exercise=TRUE, exercise.setup="model2-setup"}
XXX(XXX, 
      XXX = XXX)
```
...
```{r coeffplot2b, warning=FALSE, message=FALSE, exercise=TRUE, exercise.setup="model2-setup"}
plot_model(XXX, 
           type = XXX,
           XXX = TRUE, 
           value.offset = .3,
...
We can easily produce a plot that shows the effect of one independent variable when all other independent variables are set at their mean (any factor is set at its base level and all dichotomous variables are set to 0).
...
The `show.data` argument controls whether the raw data points (the bivariate scatterplot) are plotted. If you want to include them, set the argument to `TRUE`. If not, use `FALSE`. This is a matter of personal taste but will look cluttered if the sample is large (as here).
...
Here we will look at the effect of **white_percent**. Note that all other variables are set at their means.  Run the code and then change `show.data` to equal TRUE and run the code again to see what you learn.

```{r pred1, warning=FALSE, message=FALSE, exercise=TRUE, exercise.setup="model1-setup"}
plot_model(model1, 
           type = "pred", 
           terms = "white_percent",
           show.data = FALSE,
...
```{r pred2, warning=FALSE, message=FALSE, exercise=TRUE, exercise.setup="model2-setup"}
plot_model(model2, 
           type = "pred", 
           terms = "UrbanCat",
           show.data = FALSE,
...
```{r statetable, warning=FALSE, message=FALSE, exercise=TRUE, exercise.setup="fit1-setup"}
...
```{r statetable2, warning=FALSE, message=FALSE, exercise=TRUE, exercise.setup="fit1-setup"}
...
```{r stateplot, warning=FALSE, message=FALSE, exercise=TRUE, exercise.setup="fit1-setup"}
...
```{r stateplot2, warning=FALSE, message=FALSE, exercise=TRUE, exercise.setup="fit1-setup"}
...
```{r statepred1, warning=FALSE, message=FALSE, exercise=TRUE, exercise.setup="fit1-setup"}
...
Now plot the predicted effects of census region from the regression given in **fit1**, holding all other variables at their means. Specify the title, axis.title arguments and show the data.


```{r statepred2, warning=FALSE, message=FALSE, exercise=TRUE, exercise.setup="fit1-setup"}
